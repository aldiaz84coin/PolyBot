// lib/market-fetch.js — v1.2
//
// CAMBIOS v1.2 — FIX ReferenceError: slugs_tried is not defined
//   - formatMarket recibe el parámetro como `slugsTried` (camelCase).
//     Dentro del objeto `_debug` se usaba `slugs_tried` como shorthand
//     ({ slugs_tried }) → JavaScript buscaba una variable llamada `slugs_tried`
//     que no existe en ese scope → ReferenceError propagado hasta fetchActiveMarket
//     → catch externo lo capturaba y devolvía:
//       error: "fetchActiveMarket excepción: slugs_tried is not defined"
//     Corregido a forma explícita: { slugs_tried: slugsTried } en ambos paths
//     (return normal y catch interno de formatMarket).
//
...
//   - formatMarket: envuelto en try/catch individual; si falla devuelve objeto mínimo.
...
function formatMarket(raw, tokens, slugsTried, now) {
  ...
  try {
    ...
    return {
      ...
      _debug: {
        slugs_tried:   slugsTried,   // FIX v1.2: era `slugs_tried` (shorthand) → ReferenceError
    ...
    return {
      ...
      _debug: {
        slugs_tried:  slugsTried,   // FIX v1.2: ídem
        found_slug:   raw?.slug ?? null,
