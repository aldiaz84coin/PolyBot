 ...
 *       || captura tanto null/undefined como string vacío.
...
//
// Estrategia de fuentes (v3.4 / v3.5):
...
// cuando está fresco. Siempre se setea un error explícito si market es null,
// nunca se deja el par (market=null, error=null/falsy) que causa "Buscando..."
// ─────────────────────────────────────────────────────────────────────────────
    ...
    try {
      ...
      if (marketApi) {
        ...
        if (marketApi.active === false) {
          // FIX v3.5: usar || en vez de ?? — captura string vacío además de null/undefined
          setError(marketApi.error || "Mercado BTC no encontrado");
        } else if (!marketApi.market) {
          // Caso borde: active=true pero market=null/undefined.
          // Sin este guard, setError(null) dejaba market=null+error=null → "Buscando..."
      ...
      // FIX v3.5: limpiar data stale para no mostrar mercado anterior + error
      setData(null);
      setError("No se pudo obtener el mercado activo");

    } catch (e) {
      // FIX v3.5: || en vez de ?? — e.message puede ser string vacío en algunos entornos
