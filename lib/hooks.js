 ...
 *
 * CAMBIOS v4.1:
 *   - useBotState(): expone simulate_mode en el return.
 *     Antes: el campo llegaba en state?.raw pero no estaba accesible
 *     directamente → ModeSelector leía botState.simulate_mode === undefined.
 *     Ahora: simulate_mode: state?.simulate_mode ?? null se expone igual
 *     que el resto de campos.
 *
 * CAMBIOS v4.0 (referencia):
 *   1. useBotState() — hook dedicado para /api/bot-state.
 *   2. useMarket() — acepta botState como parámetro opcional.
 *   3. normalizeTokens() — sin cambios, v3.7.
 *
 * EXPORTS:
 *   useBotState()              → { status, running, stale, simulate_mode, target, slug, market, ... }
...
export function useBotState() {
  ...
  return {
    ...
    running,
    stale,
    simulate_mode: state?.simulate_mode ?? null,   // v4.1 — true=simulado, false=real, null=desconocido
