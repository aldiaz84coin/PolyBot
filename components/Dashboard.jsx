...
import {
  useBTCPrice, useMarket, useClock, useLog, useBalance,
} from "../lib/hooks";
...
export default function Dashboard() {
  ...
  const { market, minsLeft: minsLeftAPI }  = useMarket(running);
  const now    = useClock();
  const { log, add: addLog } = useLog();
  const { balance, pnlDay, applyBet, applyResult } = useBalance(500);

  // Usa minsLeft del API de Polymarket si disponible, sino reloj local
  const minsLeft    = minsLeftAPI ?? getMinsLeft(now);
  const activeWindow = running ? getActiveWindow(minsLeft) : null;
  const umbral       = activeWindow ? config[activeWindow.configKey] : null;
  const decision     = (running && activeWindow && price) ? getDecision(price, market?.target ?? 0, umbral) : null;

  // Target: open de la vela 1H (se resetea cada hora en punto)
  const [target, setTarget] = useState(null);
  const targetHour = useRef(null);
  useEffect(() => {
    if (!price) return;
    const h = now.getHours();
    if (targetHour.current !== h) {
      targetHour.current = h;
      setTarget(price);
      addLog(`🕐 Nueva hora — Target fijado: ${fmtUSD(price)}`, "info");
    }
  }, [now.getHours(), price]);
  ...
  useEffect(() => {
    if (!price) return;
    const ts = now.toLocaleTimeString("es-ES", { hour12: false, minute: "2-digit", second: "2-digit" });
    setPriceHistory(h => [...h.slice(-59), { ts, price, target }]);
  }, [price]);

  // ── Lógica del bot (disparo único por ventana) ───────────────────────────
  const firedWindow = useRef(null);

  useEffect(() => {
    if (!activeWindow) { firedWindow.current = null; return; }
  }, [activeWindow?.key]);

  useEffect(() => {
    if (!running || !activeWindow || !decision?.signal) return;
    if (firedWindow.current === activeWindow.key) return; // ya disparamos en esta ventana
    ...
    const dir = decision.dir === "UP" ? "▲ UP" : "▼ DOWN";
    addLog(
      `${dir} ejecutado — Entry: ${fmtUSD(price)} | Target: ${fmtUSD(target)} | Dist: $${Math.abs(decision.dist).toFixed(0)} | Ventana: ${activeWindow.key}`,
      "success"
    );
  ...
  useEffect(() => {
    ...
    const currentPnl = activeBet.dir === "UP"
      ? ((price - activeBet.entry) / activeBet.entry) * 100
      : ((activeBet.entry - price) / activeBet.entry) * 100;

    if (currentPnl <= -config.stop_loss_pct) {
      const closed = { ...activeBet, result: "STOP", pnl: -config.stop_loss_pct };
      setBets(b => b.map(bet => bet.id === activeBet.id ? closed : bet));
      setActiveBet(null);
      applyResult(activeBet.stake, false, config.stop_loss_pct);
      addLog(`🛑 STOP LOSS activado — P&L: -${config.stop_loss_pct}%`, "error");
      fetch("/api/bets", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: activeBet.id, result: "STOP", pnl: -config.stop_loss_pct }) }).catch(() => {});
    }
  }, [price, activeBet, running]);

  // ── Simulación de resolución (cierre de evento) ──────────────────────────
  useEffect(() => {
    if (!running || !activeBet || !price || !target) return;
    if (minsLeft > 1.0) return; // resolver en el último minuto

    const won = activeBet.dir === "UP" ? price > activeBet.target : price < activeBet.target;
    const pnl = won ? 90 : -config.stop_loss_pct;
    const result = won ? "WIN" : "LOSS";

    const closed = { ...activeBet, result, pnl };
    setBets(b => b.map(bet => bet.id === activeBet.id ? closed : bet));
    setActiveBet(null);
    applyResult(activeBet.stake, won, config.stop_loss_pct);
    addLog(
      won ? `✅ WIN — Claim automático iniciado. P&L: +90%` : `❌ LOSS — Evento perdido.`,
      won ? "success" : "error"
    );
    fetch("/api/bets", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: activeBet.id, result, pnl }) }).catch(() => {});
  ...
  const TABS = ["dashboard", "historial", "config"];
    ...
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)" }}>
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <Tag color="#2a4a3a">v2.0</Tag>
          <Tag color="#333">POLYGON</Tag>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 20, fontSize: 12 }}>
          ...
          <span style={{ color: balance < 100 ? "var(--red)" : "var(--green)" }}>
            BAL: {fmtUSD(balance)}
          </span>
          <button
            onClick={() => {
              const next = !running;
              setRunning(next);
              addLog(next ? "🤖 Bot iniciado — monitoreando mercado." : "🛑 Bot detenido.", next ? "success" : "error");
            }}
            style={{
      ...
      {tab === "dashboard" && (
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 1, background: "var(--border)" }}>
          ...
          <div style={{ background: "var(--bg)", padding: "20px 24px" }}>
            ...
            <div style={{ fontSize: 38, fontWeight: 700, lineHeight: 1, color: price && prev ? (price >= prev ? "var(--green)" : "var(--red)") : "var(--text)", transition: "color 0.4s" }}>
              {priceLoading ? "CARGANDO..." : price ? `$${fmt(price, 2)}` : "—"}
            </div>
            <div style={{ marginTop: 10, fontSize: 11, color: "var(--muted)" }}>
              TARGET 1H: <span style={{ color: "#aaa" }}>{target ? fmtUSD(target) : "—"}</span>
            </div>
            <div style={{ marginTop: 4, fontSize: 11 }}>
              DISTANCIA:{" "}
              <span style={{ color: dist == null ? "var(--muted)" : dist > 0 ? "var(--green)" : "var(--red)", fontWeight: 700 }}>
            ...
            {decision ? (
              <>
                ...
                <div style={{ fontSize: 10, color: "var(--muted)", marginTop: 6 }}>
                  {decision.signal
                    ? `DIST $${Math.abs(decision.dist).toFixed(0)} > $${umbral} ✓ SEÑAL OK`
          ...
          <div style={{ background: "var(--bg)", padding: "16px 24px" }}>
            ...
            <p style={{ fontSize: 11, color: "#8888aa", lineHeight: 1.7, minHeight: 56, margin: 0 }}>
              {aiText}
            </p>
          </div>

          {/* Chart */}
          <div style={{ gridColumn: "1/4", background: "var(--bg2)", padding: "0 0 0 0", borderTop: "1px solid var(--border)" }}>
          ...
          <div style={{ gridColumn: "1/4", background: "#02020a", maxHeight: 180, overflowY: "auto", borderTop: "1px solid var(--border)" }}>
            ...
            {log.length === 0 ? (
              ...
              <div key={l.id} style={{
                ...
                color: l.type === "success" ? "#00bb66" : l.type === "error" ? "#cc3355" : "var(--muted)",
              }}>
