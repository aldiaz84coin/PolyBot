...
export default function Dashboard() {
  ...
  // Target: fijar al inicio de cada hora
  useEffect(() => {
    if (!price) return;
    const h = now.getHours();
    if (targetHourRef.current !== h) {
      targetHourRef.current = h;
      setTarget(price);
      addLog(`🕐 Nueva hora — Target 1H fijado: ${fmtUSD(price)}`, "info");
    }
  }, [now.getHours(), price]);
  ...
  useEffect(() => {
    ...
    setAiLoading(true);
    fetch("/api/analysis", { method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify({ price, target, dist: decision.dist, window: activeWindow.key, decision: decision.dir }) })
      ...
      {tab === "dashboard" && (
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:1, background:"var(--border)" }}>
          ...
          <div style={{ background:"var(--bg)", padding:"20px 24px" }}>
            <div style={{ fontSize:9, color:"#444", letterSpacing:"0.15em", marginBottom:8 }}>BTC/USDT — {source?.toUpperCase()??"—"}</div>
            ...
            <div style={{ marginTop:10, fontSize:11, color:"var(--muted)" }}>
              TARGET 1H: <span style={{ color:"#ccc" }}>{target ? fmtUSD(target) : "—"}</span>
            </div>
